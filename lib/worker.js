...
Communication.on('JOB', function (Job) {
  ...
  Job.Response = new Promise(function (resolve) {
    ...
    const argv = [process.execPath, eslintDirectory, '--stdin', '--format', Path.join(__dirname, 'reporter.js')];
    if (params.rulesDir) {
...
Communication.on('FIX', function (Job) {
  ...
  const argv = [process.execPath, eslintDirectory, params.filePath, '--fix'];
  if (configFile !== null) {
    argv.push('--config', resolveEnv(configFile));
  }
  process.argv = argv;
