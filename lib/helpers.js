    ...
    return Promise.all(response.map((() => {
      var _ref6 = _asyncToGenerator(function* (_ref5) {
        ...
        try {
          ...
          if (showRule) {
            const url = ruleId ? (0, _eslintRuleDocumentation2.default)(ruleId).url : '';
            ret.url = url;
            ret.description = `${(0, _escapeHtml2.default)(message)} ([${ruleId || 'Fatal'}](${url}))`;
          } else {
...
const generateInvalidTrace = (() => {
  var _ref3 = _asyncToGenerator(function* (msgLine, msgCol, msgEndLine, msgEndCol, eslintFullRange, filePath, textEditor, ruleId, message, worker) {
    ...
    return {
      severity: 'error',
      excerpt: `${(0, _escapeHtml2.default)(titleText)}. See the trace for details. ` + `[Report this!](${newIssueURL})`,
      location,
