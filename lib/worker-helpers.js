...
function getESLintDirectory(fileDir, projectPath, config) {
  if (config.eslintDir) {
    if (_path2.default.isAbsolute(config.eslintDir)) {
      return config.eslintDir;
    }
    return _path2.default.join(projectPath, config.eslintDir);
  }
  // Find directory of eslint installation. findCached will traverse up the tree
  // checking every `node_modules` directory
  return _path2.default.dirname((0, _atomLinter.findCached)(fileDir, 'node_modules/eslint'));
}

function getESLintInstance(fileDir, projectPath, config) {
  var modulesDir = getESLintDirectory(fileDir, projectPath, config);
