...
Communication.on('JOB', function(job) {
  ...
  job.Response = new Promise(function(resolve) {
    ...
    function applyRulesDir(rulesDir) {
      rulesDir = resolveEnv(rulesDir)
    ...
    if (params.rulesDir) {
      applyRulesDir(params.rulesDir);
    }
    if (params.rulesDirs) {
        params.rulesDirs.forEach(function(rulesDir) {
          applyRulesDir(rulesDir);
        });
    }
    if (configFile !== null) {
