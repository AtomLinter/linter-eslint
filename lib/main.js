      ...
      if (this.active) {
        ...
        child.on('exit-linter', shouldLive => {
          ...
          if (shouldLive) {
            ...
            this.exitNotification = atom.notifications.addWarning(
              '[Linter-ESLint] Worker died unexpectedly',
              {
                detail: 'Check your console for more info. A new worker will be spawned in ' +
                atom.config.get('linter-eslint.respawnWorkerAfter') + 'ms.',
                dismissable: false
              }
            )
            setTimeout(() =>
              initializeWorker()
            , atom.config.get('linter-eslint.respawnWorkerAfter'))
          }
        })
      }
    }
